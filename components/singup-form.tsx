...
import { Loader2 } from "lucide-react" // Wixii spinner-ka ahaa
...
export function SignUpForm({
  ...
  try {
    ...
    await setDoc(doc(db, "users", user.uid), {
      ...
      displayName: userName,
      email: email,
      createdAt: serverTimestamp(), // Records when the account was created
...
const handleGoogleSignIn = async () => {
  try {
    ...
    await setDoc(doc(db, "users", user.uid), {
      ...
      email: user.email,
      photoURL: user.photoURL,
      lastLogin: serverTimestamp(), // Track their most recent visit
    }, { merge: true }); // 'merge: true' prevents overwriting existing custom data
  ...
  return (
    <form onSubmit={handleSignUp} className={cn("flex flex-col gap-6", className)} {...props}>
      <FieldGroup>
        ...
        <Field>
          {/* SAX: Badhanka hadda wuxuu leeyahay Loading Spinner */}
